...
public void  OnPluginStart()
{
	...
	g_hServerDealWith = CreateConVar("server_deal_with", "2", "服务器碰到不允许模式的处理方式 (1更改为设置好的模式, 2踢出玩家并重启服务器).", 0, true, 1.0, true, 2.0);
...
Action DealWithGameModeChange(Handle timer)
{
	...
	{
		if(GetMaxHumanPlayers() > 5)
		{
			LogToFileEx(g_sLogPath, "超过5人，不允许的模式%s，重启服务器", g_sCurrentGameMode);
			KickAllPlayer();
			RestartServer();
