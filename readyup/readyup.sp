...
ConVar
	...
	l4d_ready_enable_sound, l4d_ready_chuckle, l4d_ready_countdown_sound, l4d_ready_live_sound, l4d_ready_autostart_sound, l4d_ready_ready_sound,
	l4d_ready_secret,
...
public void OnPluginStart()
{
	...
	l4d_ready_ready_sound		= CreateConVar("l4d_ready_ready_sound", SECRET_SOUND, "The sound that plays when a round goes on countdown");
...
public void OnMapStart()
{
	...
	l4d_ready_ready_sound.GetString(readySound,sizeof(readySound));
...
{
	if(isTeamReadyMode){
		//PrintToChatAll("团队准备模式,%d,%d,%d",inAutoStart,inReadyUp,IsPlayer(client));
		if (inReadyUp && IsPlayer(client))
		{
			ReadyTeam(client);
			isPlayerReady[client] = true;
			//PrintToChatAll("client团队：%d",GetClientTeam(client));
			if (!inAutoStart && CheckFullReady())
				InitiateLiveCountdown();
			return Plugin_Handled;
		}
	}
	else if (inReadyUp && IsPlayer(client))
	{
		//PrintToChatAll("其他准备模式,%d",inAutoStart);
		isPlayerReady[client] = true;
		if (l4d_ready_secret.BoolValue)
			DoSecrets(client);
		if (!inAutoStart && CheckFullReady())
...
public Action Unready_Cmd(int client, int args)
{
	...
	else if (inReadyUp && !isAutoStartMode)
	{
		AdminId id = GetUserAdmin(client);
		bool hasflag = (id != INVALID_ADMIN_ID && GetAdminFlag(id, Admin_Ban)); // Check for specific admin flag
...
void InitiateReadyUp()
{
	...
	isTeamReadyMode = (l4d_ready_enabled.IntValue == READY_MODE_TEAM);
...
bool CheckFullReady()
{
	...
	if(isTeamReadyMode){
		if(survReadyCount>0 && infReadyCount>0)
			return true;
		else
			return false;
	}
	//PrintToChatAll("surcount: %d infcount: %d",survReadyCount,infReadyCount);
	int survLimit = survivor_limit.IntValue;
	int zombLimit = z_max_player_zombies.IntValue;
	//PrintToChatAll("surcountlimit: %d infcountlimit: %d",survLimit,infReadyCount);
...
void DoSecrets(int client)
{
	if (GetClientTeam(client) == L4D2Team_Survivor && !blockSecretSpam[client])
	{
		...
		EmitSoundToAll( readySound, client, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, 0.5);
