...
public OnPluginStart()
{
    ...
    PrepSDKCalls();
	IsTankPass = CreateGlobalForward("OnIsTankPass", ET_Ignore,Param_Cell);
...
static CallSurrenderMenu()
{
    ...
    new electables;
    //Format(temp,sizeof(temp),"%t","SurrenderAnyoneTankButme",primaryTankPlayer); 
    //AddMenuItem(surrenderMenu, "0", "我不知道谁会玩，但我不想背锅!");
    AddTranslatedMenuItem(surrenderMenu, "0", "SurrenderAnyoneTankButme",primaryTankPlayer);
    for (new i = 1; i <= MaxClients; i++)
    {
...
public TS_MenuCallBack(Handle:menu, MenuAction:action, param1, param2)
{
    ...
    decl String:number[4],String:name[64];
    ...
    if (!choice)
    {
        ...
        GetClientName(choice,name,sizeof(name));
        CPrintToChatAll("%t","SystemChangeToAPlayer",name);
    ...
    else
    {
        ...
        L4D2_ReplaceTank(primaryTankPlayer, choice);
        GetClientName(choice,name,sizeof(name));
        CPrintToChatAll("%t","PlayerChangeToPlayer",name);
...
public Action:TS_Display_Auto_MenuToTank(Handle:timer)
{
    ...
    //SetGlobalTransTarget(primaryTankPlayer);
    decl String:temp[MAX_NAME_LENGTH];
    ...
    decl String:name[MAX_NAME_LENGTH], String:number[10];
    new electables;
    //Format(temp,sizeof(temp),"%t","MyselfBeTank",primaryTankPlayer);
    AddTranslatedMenuItem(surrenderMenu, "0", "MyselfBeTank",primaryTankPlayer);
    //AddMenuItem(surrenderMenu, "0", "不给!我要自己装逼!");
    //AddMenuItem(surrenderMenu, "98", "加强AI帮我代打!");
    //AddMenuItem(surrenderMenu, "99", "随机让克!");
    //Format(temp,sizeof(temp),"%t","AnyoneTankButMe",primaryTankPlayer);
    AddTranslatedMenuItem(surrenderMenu,"99", "AnyoneTankButMe",primaryTankPlayer);
    for (new i = 1; i <= MaxClients; i++)
    {
        if (i == primaryTankPlayer) continue;
        if (!IsClientInGame(i)) continue;
...
public TS_Auto_MenuCallBack(Handle:menu, MenuAction:action, param1, param2)
{
    ...
    else if (choice == 99)  // "Anyone but me"
    {
        ...
        L4D2_ReplaceTank(primaryTankPlayer, choice);
        GetClientName(choice,name,sizeof(name));
		CPrintToChatAll("%t","ChangeToRandom",name);
    ...
    else    // choice is a specific player id
    {
        ...
        L4D2_ReplaceTank(primaryTankPlayer, choice);   
		GetClientName(choice,name,sizeof(name));
		CPrintToChatAll("%t","ChangeToPlayer",name);
