...
#define WAIT_TIME_BEFORE_SWITCH_COOP			45.0
...
bool CheckIsOfficalMap(char []mapname){
...
void CreateChangeMapTimer(const char[] mapName) {
	...
	if(CheckIsOfficalMap(strCurrentMap))
		delay=5.0;
	DataPack dp;
...
public void VoteMenuDraw(int iClient) {
	char strCurrentMap[LEN_MAP_FILENAME];
	GetCurrentMap(strCurrentMap,sizeof(strCurrentMap));
	if(iClient < 1 || IsClientInGame(iClient) == false || IsFakeClient(iClient) == true || CheckIsOfficalMap(strCurrentMap) == false)
		return;
