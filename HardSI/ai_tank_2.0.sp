...
public void OnPluginStart()
{
	...
	// 消耗相关
	g_hAllowConsume = CreateConVar("ai_TankAllowConsume", "0", "是否开启坦克消耗", CVAR_FLAG, true, 0.0, true, 1.0);
	...
	g_hAllowThrowRange = CreateConVar("ai_TankThrowRange", "200,450", "允许坦克丢石头的范围，中间用逗号隔开，不能有空格", CVAR_FLAG);
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	if (IsAiTank(client))
	{
		...
		if (IsValidSurvivor(target))
		{
			...
			if (bHasSight && targetdist <= g_hAttackRange.IntValue)
			{
				buttons |= IN_ATTACK;
				return Plugin_Changed;
			}*/
