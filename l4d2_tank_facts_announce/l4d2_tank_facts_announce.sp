...
public void OnIsTankPass(int target){
	//PrintToChatAll("让克事件发生，tankswap设为true");
	tankswap = true;
	if(target > 0){
		g_iTankClient = target;
	}
	else
	{
		g_iTankClient = FindTankClient(-1);
	}
}
...
public void OnClientDisconnect(int client)
{
	...
	{
		CreateTimer(0.5, Timer_CheckOthersTank); // Use a delayed timer due to bugs where the tank passes to another player
	}
}

public Action Timer_CheckOthersTank(Handle Timer){
	if(FindTankClient(-1)){
		g_iTankClient=FindTankClient(-1);
		char name[64];
		GetClientName(g_iTankClient,name,sizeof(name));
		CPrintToChatAll("%t","TankExchange",name);
		//CPrintToChatAll("[{green}坦克交换{default}]:{blue}原tank玩家退出游戏，寻找新的tank玩家。");
	}
}
/*
public Action Event_PAfkQ(Handle event, const char[] name, bool dontBroadcast) {
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if( 0 < client <= MaxClients && client == g_iTankClient )
	{
		CreateTimer(0.5, Timer_CheckAfkTank); // Use a delayed timer due to bugs where the tank passes to another player
    }
}

public Action Timer_CheckAfkTank(Handle Timer){
	if(FindTankClient(-1)){
		g_iTankClient=FindTankClient(-1);
		CPrintToChatAll("[{green}坦克交换测试{default}]:{blue}原tank玩家旁观，寻找新坦克玩家。");
	}
}
*/
...
{
	if(tankswap){
		//PrintToChatAll("处理让克事件");
		//int tankclient = FindTankClient(-1);
		//PrintToChatAll("处理让克事件，tankclient更改");
