...
ConVar TankHealthAdjust;
ConVar TankHpMulti;
ConVar SIHealthAdjust;
ConVar SiHpMulti;
ConVar CiSpMulti;
ConVar SiSpMore;
...
public void OnPluginStart()
{
	...
	ExtraFirstAid = CreateConVar("l4d_extra_first_aid", "1", "1: Allow extra first aid kits for extra players. 0: No extra kits.", CVAR_FLAGS, true, 0.0, true, 1.0);
	FinaleExtraFirstAid = CreateConVar("l4d_finale_extra_first_aid", "1", "1: Allow extra first aid kits for extra players when the finale is activated. 0: No extra kits.", CVAR_FLAGS, true, 0.0, true, 1.0);
	SurvivalExtraFirstAid = CreateConVar("l4d_survival_extra_first_aid", "0", "1: Allow extra medkits in survival for extra players when the round starts. Useful for certain maps. 0: No extra kits.", CVAR_FLAGS, true, 0.0, true, 1.0);
	AmmoPackStart = CreateConVar("l4d_give_ammo_pack", "0", "Give an ammo pack at round start for survivors? 1: Yes. 0: No.", CVAR_FLAGS, true, 0.0, true, 1.0);
	...
	//Management = CreateConVar("l4d_management", "1", "3: Enable teammenu & commands, 2: commands only, 1: !infected,!survivor,!join only, 0: Nothing", CVAR_FLAGS, true, 0.0, true, 4.0);
	//Stripbots = CreateConVar("l4d_stripbots", "0", "1: Remove gear from extra bots before kicking them?, 0: Do not remove gear.", CVAR_FLAGS, true, 0.0, true, 1.0);

	AutoDifficulty = CreateConVar("director_auto_difficulty", "1", "Change Difficulty", CVAR_FLAGS, true, 0.0, true, 1.0);
	TankHealthAdjust = CreateConVar("director_tank_auto","1","Enable Tank health to be adjusted. Requires director_auto_difficulty 1", CVAR_FLAGS,true,0.0,true,1.0);
	...
	CIMobAdjust = CreateConVar("director_ci_auto","1","Enable hordes to be adjusted. Requires director_auto_difficulty 1", CVAR_FLAGS,true,0.0,true,1.0);
	...
	SiSpMore = CreateConVar("director_si_more","0","In coop, spawn 1 more SI per extra player? Requires director_auto_difficulty 1 and Left 4 Downtown/Left 4 DHooks to work.", CVAR_FLAGS,true,0.0,true,1.0);
	...
	SIHealthAdjust = CreateConVar("director_si_health_auto","0","Enable SI health to be adjusted. Requires director_auto_difficulty 1", CVAR_FLAGS,true,0.0,true,1.0);
...
// ------------------------------------------------------------------------
// Event_RoundStart & Give Ammo Pack for survivors
// ------------------------------------------------------------------------
...
{
	if(FinaleExtraFirstAid.BoolValue && !StrEqual(gameMode, "survival, mutation15")) // Not in survival modes because kits/pills are given for free.
	{
		int client = GetAnyAliveSurvivor();
		int amount = GetSurvivorTeam() - 4;
...
// ------------------------------------------------------------------------
//  Spawn extra medkits in survival based on survivors on the team after the round starts. Useful for certain maps.
// ------------------------------------------------------------------------
public void Event_SurvivalStart(Event event, const char[] name, bool dontBroadcast)
{
	if(SurvivalExtraFirstAid.BoolValue && !InfectedAllowed)
	{
		int client = GetAnyAliveSurvivor();
		int amount = GetSurvivorTeam() - 4;
...
// ------------------------------------------------------------------------
//  Give an Ammo Pack at the start of a round for survivors.
// ------------------------------------------------------------------------
public Action Timer_GiveAmmoPack(Handle hTimer)
{
	if(AmmoPackStart.BoolValue)
	{
		AmmoPackTimer = null;

		int client = GetAnyAliveSurvivor();
		int random = GetRandomInt(0,19);
		if (random % 2 == 0)
		{
...
// ------------------------------------------------------------------------
// Newer FinaleEnd() Thanks to Edison1318 TLS update method of detecting safe survivors.
// ------------------------------------------------------------------------
...
{
	PrintToChatAll("\x04tank已生成，根据人数血量调整为：%d",TankHealth.IntValue);
...
public Action Timer_BotsUpdate(Handle hTimer)
{
	...
	if (AreAllInGame() == true)
	{
		...
		if (MedkitTimer == null && !MedkitsGiven && ExtraFirstAid.BoolValue && !StrEqual(gameMode, "survival, mutation15")) // Not in survival modes because kits/pills are given for free.
		{
			MedkitsGiven = true;
			MedkitTimer = CreateTimer(2.0, Timer_SpawnExtraMedKit);
		}
		
		if (AmmoPackTimer == null && !AmmoPackGiven && AmmoPackStart.BoolValue)
		{
			AmmoPackGiven = true;
			AmmoPackTimer = CreateTimer(2.0, Timer_GiveAmmoPack);
		}
...
void AutoDifficultyCheck()
{
	...
	if(TankHealthAdjust.BoolValue)// Should Tank health be adjusted?
	{
		...
		PrintToConsoleAll("[SV] - Tank HP: %.0f%%\t", 100.0*TankHp_Multi);
	}

	if(CIMobAdjust.BoolValue)// Should hordes be adjusted?
	{
		...
		if(ITHordeTimers.BoolValue)// Should timers be affected too?
		{
			...
			PrintToConsoleAll("[SV] - CI spawn timers are adjusted");
		}
		PrintToConsoleAll("[SV] - CI spawn rate: %.0f%%\t", 100.0*spawn_multi);
	}
	
	if(SIHealthAdjust.BoolValue)
	{
		...
		PrintToConsoleAll("[SV] - SI HP: %.0f%%\t", 100.0*sihp_Multi);
	}

	// Increase limit of special infected as bots. 
	if(!InfectedAllowed && SiSpMore.BoolValue && l4dt() && !StrEqual(gameMode, "survival,mutation15")) // Not in survival modes, versus, scavenge or realism versus, l4dt required
	{
		...
		PrintToConsoleAll("[SV] - MaxSpecials: %d", MaxSpecials);
	}
}
...
//		{
//			if(Stripbots.BoolValue)
//			{
//				StripWeapons(bot);
//			}
//			KickClient(bot, "Kicking unecessary bot");
//		}
//	}
//	return Plugin_Handled;
//}
...
//void StripWeapons(int client) // strip all equipment from bot if kicked
//{
//	int itemIdx;
//	for (int x = 0; x <= 4; x++)
//	{
