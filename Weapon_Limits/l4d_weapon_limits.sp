...
ConVar
	g_cWeaponLimitSwitch;

public Plugin myinfo =
{
	name = "L4D Weapon Limits",
	author = "CanadaRox, Stabby, Forgetest, A1m`, robex, 东",
...
public void OnPluginStart()
{
	...
	g_cWeaponLimitSwitch =  CreateConVar("l4d_wlimits_enable", "0", "武器限制开关", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	RegServerCmd("l4d_wlimits_add", AddLimit_Cmd, "Add a weapon limit");
	RegServerCmd("l4d_wlimits_lock", LockLimits_Cmd, "锁定武器控制规则提升搜索速度");
	RegServerCmd("l4d_wlimits_unlock", UnlockLimits_Cmd, "暂时解锁武器控制规则锁定，方便添加武器锁定，不用再重新输入之前的限制(解锁武器控制规则锁定必须是开启状态)");
	RegServerCmd("l4d_wlimits_clear", ClearLimits_Cmd, "清除所有武器锁定规则 (解锁武器控制规则锁定必须是开启状态)");
	...
	AutoExecConfig(true, "l4d_weapon_limit");
...
{
	//读取配置好的控制文档
	ServerCommand("exec weapon_limits.cfg");
}
...
{
	if (!g_cWeaponLimitSwitch.IntValue) {
		PrintToServer("武器数量控制限制系统处于关闭状态!");

		return Plugin_Handled;
	}

	if (bIsLocked) {
		PrintToServer("武器数量控制限制已经被锁定 !");

		return Plugin_Handled;
	}

	if (args < 3) {
		PrintToServer("用法: l4d_wlimits_add <limit> <ammo> <weapon1> <weapon2> ... <weaponN>\nAmmo: -1: 给主武器生成数量子弹数, 0: 不给子弹, 其他数值: 总是给予子弹 !");
...
public Action LockLimits_Cmd(int args)
{
	...
	if (bIsLocked) {
		PrintToServer("武器数量控制锁定已打开!");
	} else {
		bIsLocked = true;

		PrintToServer("开启武器数量控制锁定!");
	}
...
public Action ClearLimits_Cmd(int args)
{
	...
	PrintToChatAll("[L4D Weapon Limits] 武器数量控制限制已经全部清除!");
...
public Action UnlockLimits_Cmd(int args)
{
	if (!g_cWeaponLimitSwitch.IntValue) {
		PrintToServer("武器数量控制限制系统处于关闭状态!");
	...
	PrintToChatAll("[L4D Weapon Limits] 武器数量控制限制锁定已经暂时解除，请新规则添加完成后使用l4d_wlimits_lock重新开启锁定!");
...
public Action Hook_WeaponCanUse(int client, int weapon)
{
	...
	if (GetClientTeam(client) != L4D2Team_Survivor || !bIsLocked || !g_cWeaponLimitSwitch.IntValue) {
...
void denyWeapon(int wep_slot, LimitArrayEntry arrayEntry, int weapon, int client)
{
	...
	if ((g_iWeaponAlreadyGiven[client][weapon] != iWeaponRef || iButtonPressed & IN_USE)
		...
		PrintToChat(client, "\x01[\x05Weapon Limits\x01] 这种武器已经达到它的最大使用数量 [ \x04%d\x01 ]!", arrayEntry.LAE_iLimit);
