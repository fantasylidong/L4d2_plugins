...
ConVar g_hCvarAllow, g_hCvarMPGameMode, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarBots, g_hCvarDelay, g_hCvarFlags, g_hCvarStart, g_hCvarVoca, g_hGearTransferReal, g_hCvarAllowOutSafe;
bool g_bCvarAllow, g_bMapStarted, g_bLateLoad, g_bAllowOutSafe, g_bLeft4Dead2, g_bGearTransfer, g_bGameStart, g_bCanShove[MAXPLAYERS + 1] = {true, ...};
...
public void OnPluginStart()
{
	...
	g_hCvarAllowOutSafe  = CreateConVar(	"l4d_leftSafeArea_shove_allow",			"1",			"0= Can Shove Everywhere, 1=Only Allow shove in Safe area.", CVAR_FLAGS );
	...
	g_hCvarFlags = CreateConVar(	"l4d_survivor_shove_flags",			"",			"Empty string = All. Players with one of these flags have access to the shove feature.", CVAR_FLAGS );
	...
	g_hCvarAllowOutSafe.AddChangeHook(ConVarChanged_Allow);
	...
	RegConsoleCmd("sm_cmddebug", Cmd_info,"DisPlay Game Start or not");
...
{
	char sGameMode[16];
	if(g_bAllowOutSafe && (StrContains(sGameMode, "versus", false) == -1 || StrContains(sGameMode, "scavenge", false) == -1))
	{
...
{
	PrintToConsole(client,"%d %d",g_bGameStart ,g_bCanShove[client]);
}
...
/*
public bool FindPlayerShoveStatus()
{
	for(int i = 1; i <= MaxClients; i++)
	{
		if(IsClientConnected(i) && IsClientInGame(i) && GetClientTeam(i) == 2 && g_bCanShove[i] == false)
			return false;
	}
	return true;
}
...
void IsAllowed()
{
	...
	g_bAllowOutSafe = g_hCvarAllowOutSafe.BoolValue;
	...
	if( g_bCvarAllow == false && bCvarAllow == true && bAllowMode == true )
	{
		...
		HookEvent("round_end",			EventMissionLost);
		HookEvent("round_start",		EventMissionLost);
		HookEvent("map_transition", 	EventMissionLost);
	...
	{
		LogError("EventUnHooks");
		g_bCvarAllow = false;
		UnhookEvent("player_shoved",	Event_PlayerShoved);
		UnhookEvent("mission_lost",		EventMissionLost);
		UnhookEvent("round_end",		EventMissionLost);
		UnhookEvent("round_start",		EventMissionLost);
		UnhookEvent("map_transition", 	EventMissionLost);
		if( g_bLeft4Dead2 )
		{
