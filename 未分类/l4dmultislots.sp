...
ConVar hMaxSurvivors, hDeadBotTime, hSpecCheckInterval, 
	...
	hGiveKitSafeRoom, hGivePillsFinalStart, hNoSecondChane, hCvar_InvincibleTime;
...
bool bKill, bLeftSafeRoom, g_bStripBotWeapons, g_bSpawnSurvivorsAtStart, g_bEnableKick,
	g_bGiveKitSafeRoom, g_bGivePillsFinalStart, g_bNoSecondChane;
...
//	hMaxTeamLimit	= CreateConVar("l4d_multislots_max_teamlimit", "4", "Stop More Player than teamlimit", CVAR_FLAGS, true, 4.0, true, 8.0);
	...
	hRespawnHP 		= CreateConVar("l4d_multislots_respawnhp", 		"100", 	"Amount of HP a new 5+ Survivor will spawn with (Def 80)", CVAR_FLAGS, true, 0.0, true, 100.0);
	hRespawnBuffHP 	= CreateConVar("l4d_multislots_respawnbuffhp", 	"0", 	"Amount of buffer HP a new 5+ Survivor will spawn with (Def 20)", CVAR_FLAGS, true, 0.0, true, 100.0);
	hSpawnSurvivorsAtStart = CreateConVar("l4d_multislots_spawn_survivors_roundstart", "1", "If 1, Spawn 5+ survivor bots when round starts. (Numbers depends on Convar l4d_multislots_max_survivors)", CVAR_FLAGS, true, 0.0, true, 1.0);
	
	if ( g_bLeft4Dead2 ) {
		hFirstWeapon 		= CreateConVar("l4d_multislots_firstweapon", 		"0", 	"First slot weapon for new 5+ Survivor (1-Autoshot, 2-SPAS, 3-M16, 4-SCAR, 5-AK47, 6-SG552, 7-Mil Sniper, 8-AWP, 9-Scout, 10=Hunt Rif, 11=M60, 12=GL, 13-SMG, 14-Sil SMG, 15=MP5, 16-Pump Shot, 17=Chrome Shot, 18=Rand T1, 19=Rand T2, 20=Rand T3, 0=off)", CVAR_FLAGS, true, 0.0, true, 20.0);
		hSecondWeapon 		= CreateConVar("l4d_multislots_secondweapon", 		"1", 	"Second slot weapon for new 5+ Survivor (1- Dual Pistol, 2-Magnum, 3-Chainsaw, 4-Fry Pan, 5-Katana, 6-Shovel, 7-Golfclub, 8-Machete, 9-Cricket, 10=Fireaxe, 11=Knife, 12=Bball Bat, 13=Crowbar, 14=Pitchfork, 15=Guitar, 16=Random, 0=Only Pistol)", CVAR_FLAGS, true, 0.0, true, 16.0);
		...
		hFirstWeapon 		= CreateConVar("l4d_multislots_firstweapon", 		"0", 	"First slot weapon for new 5+ Survivor (1 - Autoshotgun, 2 - M16, 3 - Hunting Rifle, 4 - smg, 5 - shotgun, 6=Random T1, 7=Random T2, 0=off)", CVAR_FLAGS, true, 0.0, true, 7.0);
		...
		hThirdWeapon 		= CreateConVar("l4d_multislots_thirdweapon", 		"0", 	"Third slot item for new 5+ Survivor (1 - Moltov, 2 - Pipe Bomb, 3=Random, 0=off)", CVAR_FLAGS, true, 0.0, true, 3.0);
	...
	hGiveKitSafeRoom 		= CreateConVar("l4d_multislots_saferoom_extra_first_aid", "0", "If 1, allow extra first aid kits for 5+ players when in start saferoom, One extra kit per player above four. (0=No extra kits)", CVAR_FLAGS, true, 0.0, true, 1.0);
	hGivePillsFinalStart 		= CreateConVar("l4d_multislots_finale_extra_pills", "1" , "If 1, allow extra first aid kits for 5+ players when the finale is activated, One extra kit per player above four. (0=No extra kits)", CVAR_FLAGS, true, 0.0, true, 1.0);
	...
	hCvar_InvincibleTime 	= CreateConVar("l4d_multislots_respawn_invincibletime", 	"0.0", "Invincible time after new 5+ Survivor spawn by this plugin. (0=off)",  FCVAR_NOTIFY, true, 0.0);
	...
	hGivePillsFinalStart.AddChangeHook(ConVarChanged_Cvars);
...
public void OnGameFrame()
{
	if(TotalSurvivors()<iMaxSurvivors)
		for(int i=1, j = iMaxSurvivors-TotalSurvivors(); i <= j; i++ )
		{
			SpawnFakeClient();
		}
}
...
void GetCvars()
{
	...
	g_bGivePillsFinalStart = hGivePillsFinalStart.BoolValue;
...
public void Event_FinaleStart(Event event, const char[] name, bool dontBroadcast)
{
	if(g_bGivePillsFinalStart)
	{
		...
		if(amount > 0 && client > 0)
		{
			...
			{
				CheatCommand(client, "give", "pills", "");
			}
		}
	}
}
