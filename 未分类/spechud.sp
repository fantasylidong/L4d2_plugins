...
public Action ToggleSpecHudCmd(int client, int args) 
{
	...
	CPrintToChat(client, "<{olive}HUD{default}> 旁观 HUD 现在是 %s.", (bSpecHudActive[client] ? "{blue}开启状态{default}" : "{red}关闭状态{default}"));
	return Plugin_Handled;
}
...
public Action ToggleTankHudCmd(int client, int args) 
{
	...
	CPrintToChat(client, "<{olive}HUD{default}> Tank HUD 现在是 %s.", (bTankHudActive[client] ? "{blue}开启状态{default}" : "{red}关闭状态{default}"));
	...
	if (arraysize) // Only bother if someone's watching us
	{
		...
		for (int i = 0; i < arraysize; ++i)
		{
			...
			if (!bSpecHudHintShown[client])
			{
				...
				CPrintToChat(client, "<{olive}HUD{default}> 在聊天栏里输入 {green}!spechud{default} 来操控 {blue}旁观 HUD{default}.");
			}
		}
		delete specHud;
	}
	...
	if (FillTankInfo(tankHud, true)) // No tank -- no HUD
	{
		for (int i = 0; i < arraysize; ++i)
		{
			...
			if (!bTankHudHintShown[client])
			{
				...
				CPrintToChat(client, "<{olive}HUD{default}> 在聊天栏里输入 {green}!tankhud{default} 来操控 {red}Tank HUD{default}.");
			}
		}
	}
