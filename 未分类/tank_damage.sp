...
void FormatDuration(char[] duration, int length, float timestamp)
{
	...
	if (hours == 0 && minute > 0)
	{
		FormatEx(duration, length, "{green}%d分钟%d秒", minute, second);
	}
	else if (hours == 0 && minute == 0)
	{
		FormatEx(duration, length, "{green}%d秒", second);
	}
	else
	{
		FormatEx(duration, length, "{green}%d小时%d分钟%d秒", hours, minute, second);
	}
}