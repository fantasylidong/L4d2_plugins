...
#define HUD_FLAG_ALIGN_LEFT           0   // Left justify this text
...
public void OnPluginStart()
{
    ...
    g_hCvar_HUD3_X           = CreateConVar("l4d2_scripted_hud_hud3_x", "0.1", "X (horizontal) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
    g_hCvar_HUD3_Y           = CreateConVar("l4d2_scripted_hud_hud3_y", "0.2", "Y (vertical) position of the text.\nNote: setting it to less than 0.0 may cut/hide the text at screen.", CVAR_FLAGS, true, -1.0, true, 1.0);
...
void GetHUD2_Text(char[] output, int size)
{
    ...
    FormatEx(output, size, "QQ交流群：%s\n[电信服]当前生还者进度显示: [%d]",qq,boss_proximity);
}
...
void GetHUD3_Text(char[] output, int size)
{
    ...
    for (int client = 1; client <= MaxClients; client++)
    {
        ...
        if (output[0] == 0)
            FormatEx(output, size, "生还者血量[倒地次数]\n%N: %sHP[%d]", client, health,L4D_GetPlayerReviveCount(client));
        else
            Format(output, size, "%s\n%N: %sHP[%d]", output, client, health,L4D_GetPlayerReviveCount(client));
    }
}
