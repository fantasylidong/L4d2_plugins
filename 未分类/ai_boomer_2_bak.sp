...
public Action evt_AbilityUse(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		int target_num = 0;
		float self_pos[3] = {0.0}, self_eye_pos[3] = {0.0}, target_pos[3] = {0.0}, target_eye_pos[3] = {0.0};
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			if (IsClientConnected(i) && IsClientInGame(i) && GetClientTeam(i) == view_as<int>(TEAM_SURVIVOR) && IsPlayerAlive(i))
			{
				...
				if (GetVectorDistance(self_pos, target_pos) <= g_hVomitRange.FloatValue)
				{
					// 判断可视性
					...
					{
						bile_target[client][target_num] = i;
						target_num += 1;
					}
					delete hTrace;
		...
		bile_target_num[client][0] = target_num;
		CreateTimer(g_hVomitDuration.FloatValue, Timer_ResetBileTarget, client);
		// 上抬视野，可喷的生还者人数大于 0 且不在 cd 时间内，转视野和上抬视野
		// bile_target_num 0是目标总数，1是当前目标索引，2是循环次数
		// bile_target client表示boomer索引 target_num表示当前目标索引
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3])
{
	if (IsBoomer(client))
	{
		...
		GetClientEyePosition(client, self_eye_pos);
		// 靠近生还者，立即喷吐
		if ((flags & FL_ONGROUND) && has_sight && closet_survivor_dist <= RoundToNearest(0.5 * g_hVomitRange.FloatValue) && !in_bile_interval[client] && can_bile[client])
		{
			buttons |= IN_FORWARD;
			buttons |= IN_ATTACK;
		...
		if (g_hForceBile.BoolValue && (buttons & IN_ATTACK) && !in_bile_interval[client] && IsValidSurvivor(target))
		{
			...
			for (int i = 1; i <= MaxClients; i++)
			{
				if (IsClientConnected(i) && IsClientInGame(i) && GetClientTeam(i) == view_as<int>(TEAM_SURVIVOR) && IsPlayerAlive(i))
				{
					...
					if (!TR_DidHit(hTrace) || TR_GetEntityIndex(hTrace) == i)
					{
						if (GetVectorDistance(self_eye_pos, target_eye_pos) <= 150.0)
						{
		...
		// 连跳
		if (g_hAllowBhop.BoolValue && has_sight && (flags & FL_ONGROUND) && 0.1 * g_hVomitRange.FloatValue < closet_survivor_dist < 10000.0 && cur_speed > 160.0 && IsValidSurvivor(target))
		{
