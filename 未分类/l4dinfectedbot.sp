...
bool g_bCvarAllow, g_bMapStarted, g_bSafeSpawn, g_bTankHealthAdjust, g_bVersusCoop, g_bJoinableTeams, g_bCoopPlayableTank , g_bJoinableTeamsAnnounce,
	g_bCoordination, g_bInfHUD, g_bAnnounce , g_bAdminJoinInfected, g_bDisableSpawnsTank, g_bAdjustSpawnTimes, g_bCommonLimitAdjust, 
	g_bCoopInfectedPlayerFlashLight, g_bStatusAnnouncementEnable;
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_si", Console_ZLimit,"control max special zombies limit");
	RegConsoleCmd("sm_time", Console_Timer,"control special zombies spawn timer");
	...
	h_BoomerLimit = CreateConVar("l4d_infectedbots_boomer_limit", "1", "Sets the limit for boomers spawned by the plugin", FCVAR_NOTIFY, true, 0.0);
	...
	if (L4D2Version)
	{
		h_SpitterLimit = CreateConVar("l4d_infectedbots_spitter_limit", "1", "Sets the limit for spitters spawned by the plugin", FCVAR_NOTIFY, true, 0.0);
	...
	h_PlayerAddZombiesScale = CreateConVar("l4d_infectedbots_add_specials_scale", "1", "If server has more than 4+ alive players, how many special infected = 'max_specials' + [(alive players - 4) ÷ 'add_specials_scale' × 'add_specials'].", FCVAR_NOTIFY, true, 1.0);
	h_PlayerAddZombies = CreateConVar("l4d_infectedbots_add_specials", "1", "If server has more than 4+ alive players, increase the certain value to 'l4d_infectedbots_max_specials' each 'l4d_infectedbots_add_specials_scale' players joins", FCVAR_NOTIFY, true, 0.0);
	...
	h_PlayerAddTankHealth = CreateConVar("l4d_infectedbots_add_tankhealth", "1000", "If server has more than 4+ alive players, increase the certain value to 'l4d_infectedbots_default_tankhealth' each 'l4d_infectedbots_add_tankhealth_scale' players joins", FCVAR_NOTIFY, true, 0.0);
	h_InfectedSpawnTimeMax = CreateConVar("l4d_infectedbots_spawn_time_max", "16", "Sets the max spawn time for special infected spawned by the plugin in seconds.", FCVAR_NOTIFY, true, 1.0);
	h_InfectedSpawnTimeMin = CreateConVar("l4d_infectedbots_spawn_time_min", "16", "Sets the minimum spawn time for special infected spawned by the plugin in seconds.", FCVAR_NOTIFY, true, 1.0);
	...
	h_InitialSpawn = CreateConVar("l4d_infectedbots_initial_spawn_timer", "0", "The spawn timer in seconds used when infected bots are spawned for the first time in a map", FCVAR_NOTIFY, true, 0.0);
	h_HumanCoopLimit = CreateConVar("l4d_infectedbots_coop_versus_human_limit", "1", "Sets the limit for the amount of humans that can join the infected team in coop/survival", FCVAR_NOTIFY, true, 0.0);
	h_AdminJoinInfected = CreateConVar("l4d_infectedbots_admin_coop_versus", "1", "If 1, only admins can join the infected team in coop/survival. (Access: generic_admin)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	h_AdjustSpawnTimes = CreateConVar("l4d_infectedbots_adjust_spawn_times", "0", "If 1, The plugin will adjust spawn timers depending on the gamemode", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	h_SpawnDistanceMin = CreateConVar("l4d_infectedbots_spawn_range_min", "0", "The minimum of spawn range for infected. (default: 550, coop/realism only)", FCVAR_NOTIFY, true, 0.0);
	h_SpawnDistanceMax = CreateConVar("l4d_infectedbots_spawn_range_max", "1100.0", "The maximum of spawn range for infected. (default: 1500, coop/realism only)", FCVAR_NOTIFY, true, 1.0);
...
void GetCvars()
{
	...
	DisplayTimer = CreateTimer(1.0,ColdDown_Timer);
}
...
public Action Timer_PluginStart(Handle timer)
{
	...
	#if DEBUG
		PrintToChatAll("[电信服] PluginStart()!");
	#endif
...
public void evtPlayerFirstSpawned(Event event, const char[] name, bool dontBroadcast)
{
	...
	#if DEBUG
		PrintToChatAll("[电信服] Player has spawned for the first time");
	#endif
...
public Action Timer_InfectedKillSelf(Handle Timer, int client)
{
	...
	{
		PrintHintText(client,"[电信服] %T","Not allowed to respawn",client);
...
public void evtSurvivalStart(Event event, const char[] name, bool dontBroadcast)
{
	if (g_iCurrentMode == 3 || SurvivalVersus)
	{
		...
		{
			#if DEBUG
				PrintToChatAll("[电信服] A player triggered the survival event, spawning bots");
			#endif
			b_LeftSaveRoom = true;
...
public void evtUnlockVersusDoor(Event event, const char[] name, bool dontBroadcast)
{
	...
	#if DEBUG
		PrintToChatAll("[电信服] Attempting to spawn tempbot");
	#endif
...
public Action CheckGameMode(int client, int args)
{
	if (client)
	{
		PrintToChat(client, "[电信服] g_iCurrentMode = %i", g_iCurrentMode);
	}
...
public Action CheckQueue(int client, int args)
{
	...
	if (client)
	{
		...
		PrintToChat(client, "[电信服] InfectedBotQueue = %i, InfectedBotCount = %i, InfectedRealCount = %i", InfectedBotQueue, InfectedBotCount, InfectedRealCount);
	}
	...
	if (client && (g_iCurrentMode == 1 || g_iCurrentMode == 3) && g_bJoinableTeams)
	{
		if ((g_bAdminJoinInfected && IsPlayerGenericAdmin(client)) || !g_bAdminJoinInfected)
		{
			if (HumansOnInfected() < g_iHumanCoopLimit)
			{
				...
				PrintHintText(client, "[电信服] The Infected Team is full.");
		}
	}
...
public Action ForceInfectedSuicide(int client, int args)
{
	...
	if (client && GetClientTeam(client) == 3 && !IsFakeClient(client) && IsPlayerAlive(client) && !IsPlayerGhost(client))
	{
		...
		{
			PrintHintText(client,"[电信服] %T","Not allowed to suicide during current mode",client);
			return Plugin_Handled;
		}

		if(GetEngineTime() - fPlayerSpawnEngineTime[client] < SUICIDE_TIME)
		{
			PrintHintText(client,"[电信服] %T","Not allowed to suicide so quickly",client);
			return Plugin_Handled;
		}

		if( L4D_GetSurvivorVictim(client) != -1 )
		{
			PrintHintText(client,"[电信服] %T","Not allowed to suicide",client);
			return Plugin_Handled;
		}
...
public Action Console_ZLimit(int client, int args)
{
	...
	if (client == 0)
	{
		PrintToServer("[电信服] sm_zlimit cannot be used by server.");
		return Plugin_Handled;
	}
	if(args > 1)
	{
		ReplyToCommand(client, "[电信服] %T","Usage: sm_zlimit",client);
		return Plugin_Handled;
	}
	if(args < 1)
	{
		ReplyToCommand(client, "[电信服] %T\n%T","Current Special Infected Limit",client, g_iMaxPlayerZombies,"Usage: sm_zlimit",client);
		return Plugin_Handled;
	}
	...
	if(IsInteger(arg1))
	{
		...
		if(newlimit>30)
		{
			ReplyToCommand(client, "[电信服] %T","why you need so many special infected?",client);
		}
		else if (newlimit<0)
		{
			ReplyToCommand(client, "[电信服] %T","Usage: sm_zlimit",client);
		...
		else if(newlimit!=g_iMaxPlayerZombies)
		{
			...
			CreateTimer(0.1, MaxSpecialsSet);
			C_PrintToChatAll("[{olive}电信服{default}] {lightgreen}%N{default}: %t", client, "Special Infected Limit has been changed",newlimit);
		...
		{
			ReplyToCommand(client, "[电信服] %T","Special Infected Limit is already",client, g_iMaxPlayerZombies);
		}
		return Plugin_Handled;
	}
	else
...
public Action Console_Timer(int client, int args)
{
	...
	if (client == 0)
	{
		PrintToServer("[电信服] sm_timer cannot be used by server.");
		return Plugin_Handled;
	}

	if(args > 2)
	{
		ReplyToCommand(client, "[电信服] %T","Usage: sm_timer",client);
		return Plugin_Handled;
	}
	if(args < 1)
	{
	...
	if(args == 1)
	{
		...
		if(IsInteger(arg1))
		{
			...
			if(DD<=0)
			{
				ReplyToCommand(client, "[电信服] %T","Failed to set timer!",client);
			}
			else if (DD > 180)
			{
				ReplyToCommand(client, "[电信服] %T","why so long?",client);
			}
			else
			{
				...
				C_PrintToChatAll("[{olive}电信服{default}] {lightgreen}%N{default}: %t",client,"Bot Spawn Timer has been changed",DD,DD);
			}
			return Plugin_Handled;
		}
		else
	...
	if (IsClientInGame(client) && (!IsFakeClient(client)))
	{
		if (g_bJoinableTeamsAnnounce && g_bJoinableTeams && g_iCurrentMode != 2)
		{
			if(g_bAdminJoinInfected)
				C_PrintToChat(client,"[{olive}电信服{default}] %T","Join infected team in coop/survival/realism(adm only)",client);
			else
				C_PrintToChat(client,"[{olive}电信服{default}] %T","Join infected team in coop/survival/realism",client);
...
public Action DisposeOfCowards(Handle timer, int coward)
{
	...
	if (coward && IsClientInGame(coward) && IsFakeClient(coward) && GetClientTeam(coward) == TEAM_INFECTED && !IsPlayerTank(coward) && IsPlayerAlive(coward))
	{
		...
		else
		{
			...
			//PrintToChatAll("[电信服] Kicked bot %N for not attacking", coward);
		}
	}
	FightOrDieTimer[coward] = null;
	return Plugin_Continue;
	...
	if (g_iCurrentMode == 2)
	{
		if (IsFakeClient(client) && !IsPlayerTank(client))
		{
			...
			if(SpawnTime < 0)
				SpawnTime = 1;
			#if DEBUG
				PrintToChatAll("[电信服] playerdeath");
			#endif
			InfectedBotQueue++;
			CreateTimer(float(SpawnTime), Spawn_InfectedBot, _, TIMER_FLAG_NO_MAPCHANGE);
		...
		#if DEBUG
			PrintToChatAll("[电信服] An infected bot has been added to the spawn queue...");
		#endif
	}
	...
	if(iAliveSurplayers >= 0 && (iAliveSurplayers != iPlayersInSurvivorTeam))
	{
		...
		if(g_bTankHealthAdjust)
		{
			...
			if(g_bCommonLimitAdjust)
			{
				...
				if(g_bStatusAnnouncementEnable) C_PrintToChatAll("[{olive}电信服{default}] %t","Current status1",iAliveSurplayers,g_iMaxPlayerZombies,zombieHP[6],h_common_limit_cvar.IntValue);
			}
			else
			{
				if(g_bStatusAnnouncementEnable) C_PrintToChatAll("[{olive}电信服{default}] %t","Current status3",iAliveSurplayers,g_iMaxPlayerZombies,zombieHP[6],g_iInfectedSpawnTimeMax);
			}
		}
		else
		{
			...
			{
				SetConVarInt(h_common_limit_cvar, g_iCommonLimit + h_PlayerAddCommonLimit.IntValue * (addition/h_PlayerAddCommonLimitScale.IntValue));
				if(g_bStatusAnnouncementEnable) C_PrintToChatAll("[{olive}电信服{default}] %t","Current status2",iAliveSurplayers,g_iMaxPlayerZombies,h_common_limit_cvar.IntValue);
			}
			else
			{
				if(g_bStatusAnnouncementEnable) C_PrintToChatAll("[{olive}电信服{default}] %t","Current status4",iAliveSurplayers,g_iMaxPlayerZombies);
			}
		}
...
public void OnClientDisconnect(int client)
{
	...
	if (GetClientTeam(client) == TEAM_INFECTED && IsPlayerAlive(client))
	{
		...
		if(SpawnTime<=0)
			SpawnTime = 1;
		#if DEBUG
			PrintToChatAll("[电信服] OnClientDisconnect");
		#endif
		respawnDelay[client] = SpawnTime;
		InfectedBotQueue++;
...
void CheckIfBotsNeeded(bool spawn_immediately)
{
	...
	if ( (g_iMaxPlayerZombies - (InfectedBotCount + InfectedRealCount + InfectedBotQueue)) > 0)
	{
		...
		{
			#if DEBUG
				LogMessage("[电信服] spawn_immediately");
			#endif
			InfectedBotQueue++;
		...
		{
			#if DEBUG
				LogMessage("[电信服] initial_spawn %.1f", g_fInitialSpawn);
			#endif
			InfectedBotQueue++;
		...
		else // server can't find a valid position, we use the normal time ..
		{
			...
			#if DEBUG
				LogMessage("[电信服] InfectedBotQueue++, %d spawntime", SpawnTime);
			#endif
...
void CountInfected_Coop()
{
	...
	for (int i=1;i<=MaxClients;i++)
	{
		...
		if (GetClientTeam(i) == TEAM_INFECTED)
		{
			...
			// If someone is a tank, count him in play
			if (!IsFakeClient(i) && IsPlayerTank(i) && IsPlayerAlive(i))
			{
				InfectedRealCount++;
				continue;
			}
	...
	for (int t=1;t<=MaxClients;t++)
	{
		...
		if (!IsFakeClient(t))
		{
			...
			if ( !IsPlayerTank(t) || !IsPlayerAlive(t) )
			{
				...
				#if DEBUG
					PrintToChatAll("[电信服] Client %i found to be valid Tank Choice", Index[IndexCount]);
				#endif
			}
		}
	}
...
public Action TankBugFix(Handle timer, int client)
{
	...
	if (IsClientInGame(client) && IsFakeClient(client) && GetClientTeam(client) == 3)
	{
		...
		if (lifestate == 0)
		{
			...
			{
				#if DEBUG
					PrintToChatAll("[电信服] Ghost BugFix");
				#endif
...
int BotTypeNeeded()
{
	...
	#endif
	#if DEBUG
		PrintToChatAll("[电信服] Determining Bot type now");
	#endif
...
{
	#if DEBUG
		PrintToChatAll("[电信服] Spawn_InfectedBot(Handle timer)");
	#endif
	...
	if(anyclient == 0)
	{
		PrintToServer("[电信服] Couldn't find a valid alive survivor to spawn S.I. at this moment.",ZOMBIESPAWN_Attempts);
	...
	else
	{
		...
		switch (bot_type)
		{
			...
			case 1: // Smoker
			{
				...
				{
					PrintToServer("[电信服] Couldn't find a Smoker Spawn position in %d tries",ZOMBIESPAWN_Attempts);
				}
			}
			case 2: // Boomer
			{
				...
				{
					PrintToServer("[电信服] Couldn't find a Boomer Spawn position in %d tries",ZOMBIESPAWN_Attempts);
				}
			}
			case 3: // Hunter
			{
				...
				{
					PrintToServer("[电信服] Couldn't find a Hunter Spawn position in %d tries",ZOMBIESPAWN_Attempts);
				}
			}
			case 4: // Spitter
			{
				...
				{
					PrintToServer("[电信服] Couldn't find a Spitter Spawn position in %d tries",ZOMBIESPAWN_Attempts);
				}
			}
			case 5: // Jockey
			{
				...
				{
					PrintToServer("[电信服] Couldn't find a Jockey Spawn position in %d tries",ZOMBIESPAWN_Attempts);
				}
			}
			case 6: // Charger
			{
				...
				{
					PrintToServer("[电信服] Couldn't find a Charger Spawn position in %d tries",ZOMBIESPAWN_Attempts);
				}
			}
			case 7: // Tank
			{
				...
				{
					PrintToServer("[电信服] Couldn't find a Tank Spawn position in %d tries",ZOMBIESPAWN_Attempts);
				}
			}
		}
	...
	// Debug print
	#if DEBUG
		PrintToChatAll("[电信服] Spawning an infected bot. Type = %i ", bot_type);
	#endif
...
public Action TimerAnnounce2(Handle timer, int client)
{
	if (IsClientInGame(client))
	{
		if (GetClientTeam(client) == TEAM_INFECTED && IsPlayerAlive(client))
		{
			C_PrintToChat(client, "[{olive}电信服{default}] %T","sm_zs",client);
		}
	}
		...
		if (IsClientInGame(i) && !IsFakeClient(i))
		{
			if ( (GetClientTeam(i) == TEAM_INFECTED))
			{
				if(IsPlayerTank(i) && g_iCurrentMode != 2)
				{
					...
					if(fus <= 60)
					{
						PrintHintText(i, "[电信服] Tank Control: %d%%%%", fus);
					}
					if(fus <= 5)
					{
						PrintHintText(i, "[电信服] %T","You don't attack survivors",i);
...
public void evtInfectedSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (client && IsClientInGame(client))
	{
		if (GetClientTeam(client) == TEAM_INFECTED)
		{
			...
			{
				CreateTimer(5.0, TimerAnnounce2, client, TIMER_FLAG_NO_MAPCHANGE);
...
void SwitchToSurvivors(int client)
{
	...
	if (bot == 0)
	{
		PrintHintText(client, "[电信服] No alive survivor bots to take over.");
		return;
	}
...
{
	if( g_bCvarAllow == false || (FinaleStarted && h_WitchSpawnFinal.BoolValue == false))
	{
	...
	if(anyclient == 0)
	{
		PrintToServer("[电信服] Couldn't find a valid alive survivor to spawn witch at this moment.",ZOMBIESPAWN_Attempts);
	}
	else if (witches < h_WitchLimit.IntValue)
	{
		...
		{
			PrintToServer("[电信服] Couldn't find a Witch Spawn position in %d tries", ZOMBIESPAWN_Attempts);
		}
	}
