...
{
	if (!DoFastQuery(0, "SELECT * FROM players WHERE 1 = 2") ||
			!DoFastQuery(0, "SELECT * FROM maps WHERE 1 = 2") ||
			!DoFastQuery(0, "SELECT * FROM timedmaps WHERE 1 = 2") ||
			!DoFastQuery(0, "SELECT * FROM settings WHERE 1 = 2"))
	{
		return false;
	}
...
public void QueryClientPointsSteamID(int Client, const char []SteamID)
{

	SQLTCallback callback = GetClientPoints;
...
public void GetClientPoints(Handle owner, Handle hndl, const char []error, any client)
{
	if (!client)
		return;
...
public int GetPlayTotalpoints(int client){
	char SteamID[64];
	GetClientRankAuthString(client, SteamID, sizeof(SteamID));
	QueryClientPointsSteamID(client, SteamID);
	return ClientPoints[client];
}
public void GetClientRankAuthString(int client, char []auth,int  maxlength)
{
