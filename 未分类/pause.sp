...
#define TRANSLATION_PAUSE "pause.phrases"
...
public void OnPluginStart()
{
	...
	pauseLimitCvar = CreateConVar("sm_pauselimit", "0", "Limits the amount of pauses a player can do in a single game.", FCVAR_NONE, true, 0.0);
...
void LoadTranslation()
{
	...
	BuildPath(Path_SM, sPath, sizeof sPath, "translations/" ... TRANSLATION_PAUSE ... ".txt");
	if (!FileExists(sPath))
	{
		SetFailState("Missing translation file \"" ... TRANSLATION_PAUSE ... ".txt\"");
	}
	LoadTranslations(TRANSLATION_PAUSE);
}
...
public void OnClientPutInServer(int client)
{
	if (isPaused)
	{
		...
		{
			CPrintToChatAll("%t", "PausePLayerConnected", client);
...
public Action Pause_Cmd(int client, int args)
{
	...
	if (pauseDelay == 0 && !isPaused)
	{
		...
		CPrintToChatAll("%t", "PlayerPause", client);
...
public Action PauseDelay_Timer(Handle timer)
{
	...
	{
		CPrintToChatAll("%t", "PauseDelayTime", pauseDelay);
...
public Action ForcePause_Cmd(int client, int args)
{
	if (!isPaused)
	{
		...
		initiatorId = GetClientUserId(client);
		GetClientName(client, initiatorName, sizeof(initiatorName));
		CPrintToChatAll("%t", "ForcePauseAdmin", client);
	...
	if (isPaused && IsPlayer(client))
	{
		...
		if (!teamReady[clientTeam])
		{
			switch (clientTeam)
			{
				...
				{
					CPrintToChatAll("%t", "SurvivorReady", client, (initiatorReady && client == initiator) ? "{default}as {green}Initiator " : "", L4D2_TeamName[clientTeam]);
				...
				{
					CPrintToChatAll("%t", "InfectedReady", client, (initiatorReady && client == initiator) ? "{default}as {green}Initiator " : "", L4D2_TeamName[clientTeam]);	
		...
		if (initiatorReadyCvar.BoolValue)
		{
			if (client == initiator && !initiatorReady)
			{
				...
				if (teamReady[clientTeam])
				{
					CPrintToChatAll("%t", "InitiatorReady", client);
		...
		if (CheckFullReady())
		{
			...
			{
				CPrintToChatAll("%t", "WaitAdminUnpause");
	...
	if (isPaused && IsPlayer(client))
	{
		...
		if (teamReady[clientTeam])
		{
			switch (clientTeam)
			{
				case L4D2Team_Survivor:
				{
					CPrintToChatAll("%t", "SurvivorUnready", client, (initiatorReady && client == initiator) ? "{default}as {green}Initiator " : "", L4D2_TeamName[clientTeam]);
				...
				case L4D2Team_Infected:
				{
					CPrintToChatAll("%t", "InfectedUnready", client, (initiatorReady && client == initiator) ? "{default}as {green}Initiator " : "", L4D2_TeamName[clientTeam]);	
		...
		if (initiatorReadyCvar.BoolValue)
		{
			if (client == initiator && initiatorReady)
			{
				...
				if (!teamReady[clientTeam])
				{
					CPrintToChatAll("%t", "InitiatorUnready", client);
...
public Action ForceUnpause_Cmd(int client, int args)
{
	...
	{
		adminPause = true;
		CPrintToChatAll("%t", "AdminUnpause", client);
...
void AttemptPause()
{
	if (deferredPauseTimer == null)
	{
		...
		{
			CPrintToChatAll("%t", "AttemptPause");		
		...
		if (IsClientInGame(client) && !IsFakeClient(client))
		{
			...
			if (team == L4D2Team_Infected && IsInfectedGhost(client))
			{
				...
				if (buttons & IN_ATTACK)
				{
					...
					CPrintToChat(client, "%t", "DisableSpawn");
...
void UpdatePanel()
{
	...
	if (adminPause)
	{
		...
		menuPanel.DrawText(teamReady[L4D2Team_Survivor] ? "☑ Survivors" : "☐ Survivors");
		menuPanel.DrawText(teamReady[L4D2Team_Infected] ? "☑ Infected" : "☐ Infected");
	}
	else if (initiatorReadyCvar.BoolValue)
	{
		menuPanel.DrawText(initiatorReady ? "☑ Initiator" : "☐ Initiator");
		menuPanel.DrawText(teamReady[L4D2Team_Survivor] ? "☑ Survivors" : "☐ Survivors");
		menuPanel.DrawText(teamReady[L4D2Team_Infected] ? "☑ Infected" : "☐ Infected");
	} 
	else
...
void InitiateLiveCountdown()
{
	...
	{
		CPrintToChatAll("%t","InititeLiveCountDown");		
...
public Action ReadyCountdownDelay_Timer(Handle timer)
{
	...
	{
		Unpause();
		PrintHintTextToAll("%t","UnpauseStart");
		//PrintHintTextToAll("Game is live!");
		return Plugin_Stop;
	}
	else
	{
		CPrintToChatAll("%t","UnpauseDelayTime", readyDelay);	
...
void CancelFullReady(int client)
{
	if (readyCountdownTimer != null && !adminPause)
	{
		...
		CPrintToChatAll("%t","CancelFullReady", client);	
...
public Action Callvote_Callback(int client, char[] command, int argc)
{
	if (GetClientTeam(client) == L4D2Team_Spectator)
	{
		CPrintToChat(client, "%t", "SpecNoVote");
	...
	if (SpecTimer[client])
	{
		CPrintToChat(client, "%t", "WaitToVote");
	...
	CPrintToChat(client, "%t", "CannotKickAdmin");
