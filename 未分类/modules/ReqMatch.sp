...
RM_Match_Load()
{
	...
	if(!RM_bIsPluginsLoaded)
	{
		...
		//CPrintToChatAll("{blue}[{default}Confogl{blue}] {default}卸载模式配置!");
	...
	CPrintToChatAll("{blue}[{default}Confogl{blue}] {default}加载模式配置!");
	
	if(!RM_bIsMapRestarted && GetConVarBool(RM_hDoRestart))
	{
		CPrintToChatAll("{blue}[{default}Confogl{blue}] {default}重新启动地图!");
...
RM_UpdateCfgOn(const String:cfgfile[])
{
	...
	{
		CPrintToChatAll("{blue}[{default}Confogl{blue}] {default}启用 {olive}%s {default}配置", cfgfile);
