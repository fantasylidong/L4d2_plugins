...
public Hunter_OnModuleStart() {
	...
	SetConVarInt(hCvarHunterPounceReadyRange, 1000);
	...
	hCvarFastPounceProximity = CreateConVar("ai_fast_pounce_proximity", "2000", "At what distance to start pouncing fast");
...
public Action Hunter_OnPlayerRunCmd(hunter, &buttons, &impulse, Float:vel[3], Float:eyeAngles[3], &weapon) {	
	...
	if( (flags & FL_DUCKING) && (flags & FL_ONGROUND) ) {		
		...
		bool bHasLOS = view_as<bool>(GetEntProp(smoker, Prop_Send, "m_hasVisibleThreats")); // Line of sight to survivors		
