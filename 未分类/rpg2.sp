#pragma newdecls required强制新语法格式。
/**/
#pragma semicolon 1
#include <sourcemod>
#include <sdktools>
...
public void  OnPluginStart()
{
	...
	HookEvent("finale_win", EventMissionLost);
	HookEvent("map_transition", EventMissionLost1);
...
public Action EventMissionLost1(Handle event, const char []name, bool dontBroadcast){
	if(CheckAllMoney())
		RewardScore();
	for(int i=1;i<MaxClients;i++){
		if(IsSurvivor(i))
...
public void RewardScore(){
	...
	if(StrContains(pluginsname,"anne")==-1)
		return;
	int infectednumber=GetConVarInt(FindConVar("l4d_infected_limit"));
	if(infectednumber==5)
		AddReward(200);
	if(infectednumber==6)
		AddReward(500);
...
public void AddReward(int Score){
	...
	PrintToChatAll("\x03[RANK]幸存者强势通过当前关卡，没花费任何B数，获得额外\x04%d\x03积分",Score);
}
...
public Action BuyAmmo(int client,int args)
{
	if(IsVaildClient(client) && IsPlayerAlive(client)  )
	{
    	...
    	PrintToChat(client,"\x03你偷偷的快速补充了子弹");
	}
}
//快速买喷子指令
...
public bool CheckAllMoney(){
	for(int i=1;i<=MaxClients;i++)
	{
		if(IsValidClient(i)&&IsSurvivor(i)&&ClientPoints[i]!=500)
			return false;
	}
	return true;	
}
...
public void BuildMenu(int client)
{
	if( IsVaildClient(client) )
	{
		...
		FormatEx(binfo, sizeof(binfo), "%T", "购买枪械", client);	//武器
		menu.AddItem("gun", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "购买补给", client); //补给
		menu.AddItem("supply", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "出门近战技能", client); //技能菜单
		menu.AddItem("ability", binfo);
		if (GetConVarBool(ReturnBlood)){
			FormatEx(binfo, sizeof(binfo), "%T", "回血技能", client); //技能菜单
...
public void gun(int client)
{
	if( IsVaildClient(client) )
	{
		...
		if(ClientFirstBuy[client]){
			FormatEx(binfo, sizeof(binfo), "%T", "普通小手枪",0, client);
			menu.AddItem("pistol", binfo);

			FormatEx(binfo, sizeof(binfo), "%T", "马格南",0, client);
			menu.AddItem("pistol_magnum", binfo);
			FormatEx(binfo, sizeof(binfo), "%T", "Uzi",0, client);
			menu.AddItem("smg", binfo);

			FormatEx(binfo, sizeof(binfo), "%T", "消音smg",0,client);
			menu.AddItem("smg_silenced", binfo);
		
			FormatEx(binfo, sizeof(binfo), "%T", "一代单发霰弹枪",0, client);
			menu.AddItem("pumpshotgun", binfo);

			FormatEx(binfo, sizeof(binfo), "%T", "二代单发霰弹枪",0, client);
			menu.AddItem("shotgun_chrome", binfo);
		}else{
			FormatEx(binfo, sizeof(binfo), "%T", "普通小手枪",CostP220, client);
			menu.AddItem("pistol", binfo);

			FormatEx(binfo, sizeof(binfo), "%T", "马格南",CostMagnum, client);
			menu.AddItem("pistol_magnum", binfo);
			FormatEx(binfo, sizeof(binfo), "%T", "Uzi",CostUzi, client);
			menu.AddItem("smg", binfo);

			FormatEx(binfo, sizeof(binfo), "%T", "消音smg",CostSilenced,client);
			menu.AddItem("smg_silenced", binfo);
		
			FormatEx(binfo, sizeof(binfo), "%T", "一代单发霰弹枪",CostPumpShotgun, client);
			menu.AddItem("pumpshotgun", binfo);

			FormatEx(binfo, sizeof(binfo), "%T", "二代单发霰弹枪",CostPumpShotgun, client);
			menu.AddItem("shotgun_chrome", binfo);
		}
		FormatEx(binfo, sizeof(binfo), "%T", "普通小手枪",CostP220, client);
		menu.AddItem("pistol", binfo);
		...
		FormatEx(binfo, sizeof(binfo), "%T", "mp5机枪",CostMP5, client);
		menu.AddItem("smg_mp5", binfo);
		
		FormatEx(binfo, sizeof(binfo), "%T", "一代连发霰弹枪",CostAuto, client);
		menu.AddItem("autoshotgun", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "二代连发霰弹枪",CostAuto, client);
		menu.AddItem("shotgun_spas", binfo);
		
		FormatEx(binfo, sizeof(binfo), "%T", "m16步枪",CostM16, client);
		menu.AddItem("rifle", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "ak47步枪",CostAK47, client);
		menu.AddItem("rifle_ak47", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "sg552步枪",CostSG552, client);
		menu.AddItem("rifle_sg552", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "scar步枪",CostSCAR, client);
		menu.AddItem("rifle_desert", binfo);
		
		FormatEx(binfo, sizeof(binfo), "%T", "一代连狙",CostHunting, client);
		menu.AddItem("hunting_rifle", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "二代连狙",CostMilitary, client);
		menu.AddItem("sniper_military", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "鸟狙",CostScout, client);
		menu.AddItem("sniper_scout", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "AWP狙击枪",CostAWP, client);
		menu.AddItem("sniper_awp", binfo);
		
		FormatEx(binfo, sizeof(binfo), "%T", "m60",CostM60, client);
		menu.AddItem("rifle_m60", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "榴弹发射器",CostGrenadeLuanch, client);
		...
		case MenuAction_Select:
		{
			...
			if( StrEqual(bitem, "smg") )
			{
				...
				if(ClientFirstBuy[param1]){
					...
					PrintToChatAll("\x04%N\x03第一次白嫖了一把%t，还剩%d的B数",param1,"Uzi",ClientPoints[param1]);
				}
				else if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostUzi,"Uzi",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "smg_silenced") )
			{
				...
				if(ClientFirstBuy[param1]){
					ClientFirstBuy[param1]=false;
					RemovePoints(param1, 0, bitem);
					PrintToChatAll("\x04%N\x03第一次白嫖了一把%t，还剩%d的B数",param1,"消音smg",ClientPoints[param1]);
				}
				else if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostSilenced,"消音smg",ClientPoints[param1]);
			}				
			else if( StrEqual(bitem, "smg_mp5") )
			{
				...
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostMP5,"mp5",ClientPoints[param1]);
			}	
			else if( StrEqual(bitem, "rifle") ){
				//
				int costpoints = CostM16;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostM16,"m16步枪",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "rifle_ak47") ){
				int costpoints = CostAK47;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostAK47,"ak47步枪",ClientPoints[param1]);
			}
				
			else if( StrEqual(bitem, "rifle_sg552") ){
				int costpoints = CostSG552;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostSG552,"sg552步枪",ClientPoints[param1]);
			}
				
			else if( StrEqual(bitem, "rifle_desert") ){
				int costpoints = CostSCAR;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostSCAR,"scar步枪",ClientPoints[param1]);
			}
				...
				if(ClientFirstBuy[param1]){
					ClientFirstBuy[param1]=false;
					RemovePoints(param1, 0, bitem);
					PrintToChatAll("\x04%N\x03第一次白嫖了一把%t，还剩%d的B数",param1,"一代单喷",ClientPoints[param1]);
				}
				else if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostPumpShotgun,"一代单喷",ClientPoints[param1]);
			}
				...
				if(ClientFirstBuy[param1]){
					ClientFirstBuy[param1]=false;
					RemovePoints(param1, 0, bitem);
					PrintToChatAll("\x04%N\x03第一次白嫖了一把%t，还剩%d的B数",param1,"二代单喷",ClientPoints[param1]);
				}
				else if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostChromeShotgun,"二代单喷",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "autoshotgun") ){
				int costpoints = CostAuto;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostAuto,"一代连喷",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "shotgun_spas") ){
				int costpoints = CostSPAS;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostSPAS,"二代连喷",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "hunting_rifle") ){
				int costpoints = CostHunting;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostHunting,"一代连狙",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "sniper_military") ){
				int costpoints = CostMilitary;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostMilitary,"二代连狙",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "sniper_scout") ){
				int costpoints = CostScout;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostScout,"鸟狙",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "sniper_awp") ){
				int costpoints = CostAWP;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostAWP,"AWP狙击枪",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "rifle_m60") ){
				int costpoints = CostM60;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostM60,"m60",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "grenade_launcher") ){
				int costpoints = CostGrenadeLuanch;
				RemovePoints(param1, costpoints, bitem);
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostGrenadeLuanch,"榴弹发射器",ClientPoints[param1]);
			}
				...
				if(ClientFirstBuy[param1]){
					ClientFirstBuy[param1]=false;
					RemovePoints(param1, 0, bitem);
					PrintToChatAll("\x04%N\x03第一次白嫖了一把%t，还剩%d的B数",param1,"小手枪",ClientPoints[param1]);
				}
				else if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostP220,"小手枪",ClientPoints[param1]);
			}
				...
				if(ClientFirstBuy[param1]){
					ClientFirstBuy[param1]=false;
					RemovePoints(param1, 0, bitem);
					PrintToChatAll("\x04%N\x03第一次白嫖了一把%t，还剩%d的B数",param1,"马格南",ClientPoints[param1]);
				}
				else if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostMagnum,"马格南",ClientPoints[param1]);
			}
		}
		case MenuAction_End:
			delete menu;
...
public void supply(int client)
{
	if( IsVaildClient(client) )
	{
		...
		FormatEx(binfo, sizeof(binfo), "%T", "药丸",CostPills, client);
		menu.AddItem("pain_pills", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "肾上腺素",CostAdren, client);
		menu.AddItem("adrenaline", binfo);
		
		FormatEx(binfo, sizeof(binfo), "%T", "医疗包",CostFirstAidKit,client);
		menu.AddItem("first_aid_kit", binfo);
		
		FormatEx(binfo, sizeof(binfo), "%T", "油桶",CostGascan, client);
		menu.AddItem("gascan", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "子弹",CostAmmo, client);
		menu.AddItem("ammo", binfo);
		
		FormatEx(binfo, sizeof(binfo), "%T", "治疗小侏儒",CostGnome, client);
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			if( StrEqual(bitem, "first_aid_kit") ){				
				...
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostFirstAidKit,"医疗包",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "pain_pills") ){				
				int costpoints = CostPills;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostPills,"药丸",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "adrenaline") ){				
				int costpoints = CostAdren;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostAdren,"肾上腺素",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "gascan") ){
				
				int costpoints = CostGascan;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostGascan,"油桶",ClientPoints[param1]);
			}
			else if( StrEqual(bitem, "weapon_gnome") ){
				
				int costpoints = CostGnome;
				if(RemovePoints(param1, costpoints, bitem))
				PrintToChatAll("\x04%N\x03B数-%d,购买了%t，还剩%d的B数",param1,CostGnome,"治疗小侏儒",ClientPoints[param1]);
			}
...
public void ability(int client)
{
	if( IsVaildClient(client) )
	{
		...
		FormatEx(binfo, sizeof(binfo), "%T", "砍刀", client);
		menu.AddItem("machete", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "消防斧", client);
		menu.AddItem("fireaxe", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "小刀", client);
		menu.AddItem("knife", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "武士刀", client);
		menu.AddItem("katana", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "马格南", client);
		menu.AddItem("pistol_magnum", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "电吉他", client);
		menu.AddItem("electric_guitar", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "警棍", client);
		menu.AddItem("tonfa", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "草叉", client);
		menu.AddItem("pitchfork", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "铲子", client);
...
public void Blood(int client)
{
	if( IsVaildClient(client) )
	{
		...
		FormatEx(binfo, sizeof(binfo), "%T", "是", client);
		menu.AddItem("Yes", binfo);

		FormatEx(binfo, sizeof(binfo), "%T", "否", client);
