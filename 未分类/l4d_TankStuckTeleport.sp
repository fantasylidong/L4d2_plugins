...
public void OnPluginStart()
{
	...
	g_hCvarDestObject = CreateConVar(		"l4d_TankAntiStuck_dest_object",			"1",		"Object to teleport tank to (0 - next to current tank, 1 - next to another tank, 2 - next to player)", CVAR_FLAGS );
	...
	g_hCvarIdlerPunish = CreateConVar(		"l4d_TankAntiStuck_idler_punish",			"0",		"Punish the player who doesn't move by teleporting nearest tant to him? (0 - No / 1 - Yes)", CVAR_FLAGS );
	g_hCvarIdlerRadius = CreateConVar(		"l4d_TankAntiStuck_idler_radius",			"30",		"Maximum radius within player moves considered as idler (stucker)", CVAR_FLAGS );
	g_hCvarIdlerCheckTimes = CreateConVar(	"l4d_TankAntiStuck_idler_check_times",		"3",		"Number of checks before finally considering player as idler (stucker)", CVAR_FLAGS );
	g_hCvarIdlerCheckInterv = CreateConVar(	"l4d_TankAntiStuck_idler_check_interval",	"3",		"Interval (in sec.) between each check for idler (stucker)", CVAR_FLAGS );
	g_hCvarIdlerMinPlayers = CreateConVar(	"l4d_TankAntiStuck_idler_minplayers",		"2",		"Minimum living players allowed for 'Idle player' rule to work", CVAR_FLAGS );
	
	g_hCvarRusherPunish = CreateConVar(		"l4d_TankAntiStuck_rusher_punish",			"1",		"Punish the player who rush too far from the nearest tank by teleporting tank to him? (0 - No / 1 - Yes)", CVAR_FLAGS );
	g_hCvarRusherDist = CreateConVar(		"l4d_TankAntiStuck_rusher_dist",			"2000",		"Maximum distance to the nearest tank considered as rusher", CVAR_FLAGS );
	g_hCvarRusherCheckTimes = CreateConVar(	"l4d_TankAntiStuck_rusher_check_times",		"3",		"Number of checks before finally considering player as rusher", CVAR_FLAGS );
	g_hCvarRusherCheckInterv = CreateConVar("l4d_TankAntiStuck_rusher_check_interval",	"10",		"Interval (in sec.) between each check for rusher", CVAR_FLAGS );	
