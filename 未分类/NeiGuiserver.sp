...
public void OnPluginStart()
{
	...
	HookConVarChange(serverMaxSpawnTimeConvar, CvarEvent_PlayerMaxSpawnTimeChange);
...
public Action InfectedPlayerSpawn(Handle timer, int client){
	SetSeenSurvivorsState(client,true);
	SetSpawnFlags(client,GetSpawnFlags(client)&~4);
	Event_CreateClassMenu(client);
}
//获得Bot生还
int FindInfectBot(int Client)
{
    for (int client = 1; client <= MaxClients; client++)
        if (IsClientInGame(client) && IsFakeClient(client) && IsInfected(client))
            return client;
    GhostTime[Client]=GhostTime[Client]+1;
	return -1;
}

//获取幽灵特感，超过接管时间强制创造特感
public Action GetGhostSi(Handle timer,int client){
		int i=FindInfectBot(client);
		if(i>0&&i<=MaxClients)
			{
		...
		else if(GhostTime[client]<serverMaxSpawnTime)
			CreateTimer(1,GetGhostSi,client);
			else
			CreateTimer(0.5,InfectedPlayerSpawn,client);
}
//玩家离开安全屋
...
public void CvarEvent_PlayerMaxSpawnTimeChange(ConVar convar, const char[] oldValue, const char[] newValue){
	serverMaxSpawnTime=GetConVarInt(serverMaxSpawnTimeConvar);
}
//玩家特感开关事件
