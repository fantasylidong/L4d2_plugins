...
public void OnPluginStart()
{
	...
	//HookEvent("finale_start", OnFinaleWinStart );
...
public Action ShowVoteMenu(int client, int args)
{
	...
	{
		PrintToChat(client, "\x04[FMC]\x03 仅在救援关使用!");
		return Plugin_Handled;
	}
	
	if (IsVoteInProgress())
	{
		PrintToChat(client, "\x04[FMC]\x03 投票正在进行中!");
		return Plugin_Handled;
	}
...
public Action OnFinaleWinStart(Event event, const char[] name, bool dontBroadcast)
{
	if (!bFMCEnabled || !L4D_IsMissionFinalMap())
	{
		return Plugin_Continue;
	}
	PrintToChatAll("OnFinaleWinStart");
	FindConVar("mp_gamemode").GetString(sGameMode, sizeof(sGameMode));
	if ((StrContains(sGameMode, "versus", false) != -1 && GameRules_GetProp("m_bInSecondHalfOfRound") && !voteInitiated) || ((StrEqual(sGameMode, "coop", false) || StrEqual(sGameMode, "realism", false)) && !voteInitiated))
	{
		...
		CreateTimer(0.1, AnnounceNextCamp);
		CreateTimer(3.0, VoteCampaignDelay);
	}
	return Plugin_Continue;
}
...
{
	PrintToChatAll("\x04[FMC]\x03 这是最后一关!");
	if (!StrEqual(sVotedMap, "", false))
	{
		PrintToChatAll("\x04[FMC]\x03 接下来地图: %s |%s|", sVotedMap, sVotedCamp);
	}
	else
	{
		FMC_GetNextCampaign(sCurrentMap);
		PrintToChatAll("\x04[FMC]\x03 接下来地图: %s |%s|", sNextMap, sNextCamp);
	}
	return Plugin_Stop;
}
...
public Action VoteCampaignDelay(Handle timer)
{
	...
	PrintToChatAll("\x04[FMC]\x03 在5秒后开始投票..");
...
{
	PrintToChatAll("\x04[FMC]\x03 开始投票!");
	FMCMenu();
...
)
{
	...
	PrintToChatAll("\x04[FMC]\x03 投票结果: %s (%s) \x05[%d 票]", sVotedCamp, sVotedMap, item_info[majorityItem][VOTEINFO_ITEM_VOTES]);
	voteInitiated = true;
