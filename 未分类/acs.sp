...
public void OnPluginStart() {
	...
	g_hCVar_PreventEmptyServer =  CreateConVar("acs_prevent_empty_server", "0", "If enabled, the server automatically switch to the first available official map when no one is playing a 3-rd map [0 = DISABLED, 1 = ENABLED]", _, true, 0.0, true, 1.0);	
...
public void VoteMenuDraw(int iClient) {
	if(iClient < 1 || IsClientInGame(iClient) == false || IsFakeClient(iClient) == true || CheckIsOfficalMap() == false)
		return;
