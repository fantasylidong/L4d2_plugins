...
void vRygive(int client)
{
	...
	menu.AddItem("m", "杂项");
	menu.AddItem("t", "团队控制");
	if(g_bWeaponHandling && GetUserAdmin(client).ImmunityLevel == 100)
		menu.AddItem("c", "武器操纵性");
	if(iGetClientImmunityLevel(client) > 99)
	{
		if(g_bDebug == false)
...
void vInfecteds(int client, int index)
{
	...
	if(GetUserAdmin(client).ImmunityLevel == 100)
