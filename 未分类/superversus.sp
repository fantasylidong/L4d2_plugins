...
public void OnPluginStart()
{
	...
	g_hAutoJoin = 			CreateConVar("bots_auto_join_survivor", "0", 		"玩家连接后, 是否自动加入生还者. \n0=否, 1=是.", CVAR_FLAGS);
	g_hRespawnJoin = 		CreateConVar("bots_respawn_on_join", 	"0", 		"玩家第一次进服时如果没有存活的Bot可以接管是否复活. \n0=否, 1=是.", CVAR_FLAGS);
	g_hSpecCmdLimit = 		CreateConVar("bots_spec_cmd_limit", 	"99", 		"当完全旁观玩家达到多少个时禁止使用sm_spec命令.", CVAR_FLAGS);
	g_hSpecNextNotify = 	CreateConVar("bots_spec_next_notify", 	"0", 		"完全旁观玩家点击鼠标左键时, 提示加入生还者的方式 \n0=不提示, 1=聊天栏, 2=屏幕中央, 3=弹出菜单.", CVAR_FLAGS);
	g_esWeapon[0].cFlags = 	CreateConVar("bots_give_slot0", 		"31", 	"主武器给什么. \n0=不给, 131071=所有, 7=微冲, 1560=霰弹, 30720=狙击, 31=Tier1, 32736=Tier2, 98304=Tier0.", CVAR_FLAGS);
	g_esWeapon[1].cFlags = 	CreateConVar("bots_give_slot1", 		"2", 	"副武器给什么. \n0=不给, 131071=所有.(如果选中了近战且该近战在当前地图上未解锁,则会随机给一把).", CVAR_FLAGS);
