...
public Action OnRoundEnd_Post(Handle hTimer)
{
	...
	if (g_hMapTransitionPair.GetString(sCurrentMapName, sNextMapName, sizeof(sNextMapName))) {
		...
		CPrintToChatAll("{olive}[自动换图系统]{default} 地图: {blue}%s{default} 即将更换为: {blue}%s", sCurrentMapName, sNextMapName);
...
{
	if (iArgs != 2) {
		PrintToServer("用法: sm_add_map_transition <starting map name> <ending map name>");
		LogError("用法: sm_add_map_transition <starting map name> <ending map name>");
