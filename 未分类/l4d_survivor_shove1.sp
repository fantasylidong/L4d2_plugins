...
bool g_bCvarAllow, g_bMapStarted, g_bLateLoad, g_bAllowOutSafe, g_bLeft4Dead2, g_bGearTransfer, g_bCanShove[MAXPLAYERS + 1] = {true, ...};
...
public bool FindPlayerShoveStatus()
{
	for(int i = 1; i <= MaxClients; i++)
	{
		if(IsClientConnected(i) && IsClientInGame(i) && g_bCanShove[i] == false)
			return false;
	}
	return true;
}
