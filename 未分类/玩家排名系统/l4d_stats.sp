...
public OnPluginStart()
{
	...
	cvar_SiteURL = CreateConVar("l4d_stats_siteurl", "http://sb.trygek.com:8880/l4d_stats/", "Community site URL, for rank panel display", 0);
...
public Action:timer_PanicEventEnd(Handle:timer, Handle:hndl)
{
	...
	if (PanicEvent && !PanicEventIncap)
	{
		...
		if (Score > 0)
		{
			...
			if (Mode)
				StatsPrintToChatTeam(TEAM_SURVIVORS, "\x03所有幸存者 \x01获得了 \x04%i \x01分 by \x05 Panic事件过后没有倒地或者死亡\x01!", Score);
		}
	}
...
//	}
	...
	else if (AwardID == 94) // Let infected in safe room
	{
		...
		if (Mode == 1 || Mode == 2)
			...
			StatsPrintToChatAll("\x05%s \x01失去了 \x04%i \x01分 by 让 \x03感染者进入安全门!", UserName, Score);
	...
	else if (AwardID == 98) // Round restart
	{
		...
		if (Mode)
		{
			StatsPrintToChat(User, "\x03所有幸存者 \x01都 \x03失去 \x04%i \x01分 by \x03大家又坐牢了!", Score);
		}
...
public Action:event_Award_L4D2(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (AwardID == 80) // Respawn friendly
	{
		...
		if (Score > 0)
		{
			if (Mode == 1 || Mode == 2)
			{
				StatsPrintToChat(User, "你获得 \x04%i \x01分 by 解就  \x05%s\x01!", Score, RecipientName);
			}
			else if (Mode == 3)
			{
				StatsPrintToChatAll("\x05%s \x01获得 \x04%i \x01分 by 解就 \x05%s\x01!", UserName, Score, RecipientName);
			}
		}
	}
...
public Action:event_CarAlarm(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for (new i = 1; i <= maxplayers; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i) && !IsClientBot(i) && GetClientTeam(i) == TEAM_SURVIVORS)
		{
			...
			if (Mode)
				StatsPrintToChat(i, "\x03所有幸存者 \x01都 \x03失去了 \x04%i \x01分 by \x03又特么打警报车是吧.jpg\x01!", Score);
		}
	}
}
...
public GetClientRankRankChange(Handle:owner, Handle:hndl, const String:error[], any:client)
{
	...
	if (RankChangeLastRank[client] != ClientRank[client])
	{
		...
		if (!IsClientBot(client) && IsClientConnected(client) && IsClientInGame(client))
			StatsPrintToChat(client, "你在排名榜上上升了 \x04%i 名\x01! \x05(Rank: %i)", RankChange,  RankChangeLastRank[client]);
	}
}
...
AnnounceClientRankMute(client)
{
	StatsPrintToChat2(client, true, "你 %s \x01the \x05%s\x01.", (ClientRankMute[client] ? "\x04已经禁用" : "\x03没有禁用"), PLUGIN_NAME);
}
...
public CheckSurvivorsWin()
{
	...
	if (Mode && Score > 0)
	{
		...
		if (NegativeScore)
			StatsPrintToChatTeam(TEAM_INFECTED, "\x03所有感染者 \x01 \x03LO失去了 \x04%i \x01points by 让幸存者进了安全门!", Score);
	}
